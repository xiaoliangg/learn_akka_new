...
public class Counter extends AbstractBehavior<Counter.Command> {
    ...
    private Behavior<Command> onIncrement(Increment cmd) {
        replicatorAdapter.askUpdate(
                askReplyTo ->
                        new Replicator.Update<>( // 5个参数的含义见文档，以下注释是从文档中抄下来的
                                key, // the KEY we want to update
                                GCounter.empty(), // the data to use as the empty state if the replicator has not seen the key before
                                Replicator.writeLocal(), // the write consistency level we want for the update
                                askReplyTo,  // an ActorRef<Replicator.UpdateResponse<GCounter>> to respond to when the update is completed
                                curr -> curr.increment(node, 1)), // a modify function that takes a previous state and updates it, in our case by incrementing it with 1
